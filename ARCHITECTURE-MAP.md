...
**Genbrugelige mekanismer (Session 5):**

21. [Reusable Mechanisms — slaa op FOER du bygger](#21-reusable-mechanisms--slaa-op-foer-du-bygger)
...
## 21. Reusable Mechanisms — slaa op FOER du bygger

**Hvorfor dette afsnit findes (17. juli 2026):** Michael: *"Jeg er en smule bekymret over at
du engang imellem opdager nogle funktionaliteter vi har brugt virkeligt langt tid paa at
bygge."* Han har ret. Samme dag "opdagede" jeg `bbRenderTree`, `bubbleCard`s broedkrummer,
Historik-grenen og den deaktiverede DM-upload — alle bygget for laengst, ingen af dem nemme
at finde. Konteksten forsvandt ikke; den var bare ikke skrevet ned noget sted man kigger.

Resten af dette dokument beskriver **data-lag og flows**. Dette afsnit er **UI-mekanismer**:
faerdige, delte byggeklodser. Det er ikke dokumentation af hvordan de virker — kun **at de
findes, og hvor**. Formaal: undgaa at genopfinde eller "opdage" dem igen.

**Regel:** slaa op her (og `grep`) FOER du foreslaar eller bygger UI. Hvis noget mangler i
listen, tilfoej det naar du finder det.

### 21.1 Traestruktur med guide-linjer — `bbRenderTree`

| | |
|---|---|
| **Hvor** | `b-chat.js:41` (+ `bbTreeToggleFlat`/`_bbFlatExpanded`/`_bbFlatRerender` ~249-257) |
| **Signatur** | `bbRenderTree(nodes, expandedIds, opts)` → HTML-streng |
| **nodes** | Fladt array: `{id, name, type, parent_id, member_count, event_date, event_end_date, visibility, icon_url, _live, _unread, _star, _pending, _ghost}` |
| **opts** | `{ fromScreen: 'screen-bubbles' \| 'screen-bubble-chat' }` |
| **Bruges fra** | `b-home.js:1733` (Mine bobler), `b-chat.js:2247` (boble-info-panel) |

Kan mere end man tror — tjek foer du bygger noget selv:
- **Guide-linjer** der tegner graenene korrekt (inkl. `ancHasNext`-algoritme for lodrette
  linjer gennem flere niveauer)
- **Dybde-varieret styling**: root 44px ikon / 18px radius / vaegt 700, boern 38px / 15px / 600
- **Sortering per gren**: sub-netvaerk foerst → kommende events (naermest dato) → afsluttede
- **Historik-gren**: afsluttede events samles automatisk under en foldbar "Historik (n)"-raekke
- **Ikon-farver**: events teal `#2ECFCF`, netvaerk isblaa `rgb(100,180,230)`
- Badges: LIVE, AFSLUTTET, unread-dot, stjerner

**Expand-state er DELT globalt** (`_bbFlatExpanded`). Hver skaerm saetter `_bbFlatRerender`
til sin egen re-render — `bbTreeToggleFlat(id)` opdaterer state og kalder callbacken.
Re-render er ren DOM (ingen DB-kald).

### 21.2 Boble-kort med broedkrummer — `bubbleCard`

| | |
|---|---|
| **Hvor** | `b-home.js:~2055` |
| **Signatur** | `bubbleCard(b, fromScreen)` → HTML-streng |
| **Bruges fra** | `b-bubbles.js:262` (`_renderDiscoverList`), Mine bobler m.fl. |

**Renderer allerede herkomst** (Variant A): hvis boblen har `_parentName`/`_grandparentName`
vises `[Bedsteforaelder] › [Foraelder]` som pills over titlen; kun `[Foraelder]` ved ét niveau;
intet ved enkeltstaaende. **Discover beregner allerede disse felter** (`b-bubbles.js:152-176`
bygger `parentMap`/`gpMap`). Dvs. herkomst-visning i Discover er LIVE i dag — ikke noget der
skal bygges.

### 21.3 Bekraeftelses-UI

- **`bbConfirm(targetEl, {label, confirmText, confirmClass, onConfirm})`** — inline
  bekraeftelse ved en knap (bruges af leaveBubble m.fl.). Haandterer selv Annuller.
- **DELETE-moensteret** (`b-profile.js:922-990`) — den "fool proof" procedure til
  irreversible handlinger: foldet vaek som standard + nulstilles ved aabning, skriv `DELETE`
  eksakt (case-sensitivt, samme ord DA+EN), knap `disabled` indtil match, laas mod
  dobbelt-tryk. **Genbrug denne ved boble-sletning** (se TD-005).

### 21.4 Besked-rendering

- **`dmReduceMsg(msg)`** — centraliserer ALLE DM-inserts (6 stier, dedup paa `data-msg-id`)
- **`bcReduceMsg(msg)`** — samme for boble-chat (4 stier, dedup paa `data-bc-msg-id`)
- **`dmRenderMsg`** (`b-realtime.js:~928`) / **`bcRenderMsg`** (`b-chat.js:~1446`) — selve
  raekke-HTML. Begge bruger placeholder + async signed-URL-udfyldning for private filer.

### 21.5 Private filer

- **`resolvePrivateFileUrl(stored)`** (`b-utils.js`) — fuld URL (gammel/Tenor-GIF) →
  uaendret; sti → cached signed URL (1 time) fra `bubble-private`.
- **`_isStoragePath(val)`** — skelner sti fra fuld URL.
- **`bcFillFileLink` / `dmFillFileLink`** — udfylder `[data-file-pending]`-elementer efter render.

### 21.6 Oevrige

- **`dbActions.*`** (`b-utils.js`) — write-laget, 7 funktioner. Se afsnit 2.4.
- **`escHtml(s)`** — HTML-escaping. **Bemaerk:** escaper IKKE `javascript:`-skemaet i href.
  Brug **`sanitizeHttpsUrl(raw)`** (`b-utils.js`) til URL'er — returnerer kun https, ellers null.
- **`icon(name)`** — SVG-ikoner.
- **`visIcon(visibility)`** — synligheds-ikon (haengelaas m.m.).
- **`_locale()`** — dato-lokalisering.
