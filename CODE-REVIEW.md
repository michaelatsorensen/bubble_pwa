...
- ✅ **Bug: `createBubble()` ignorerer auto-join fejl** (LUKKET v8.88) (b-utils.js:1095). Bubble-insert er error-checket, men `bubble_members`-insert er fire-and-forget → returnerer `{ok:true}` selv hvis owner IKKE blev medlem. Fake-success. **Fix (LOW risk):** check error paa member-insert, vis warning/rollback. → bør fixes (lille, afgraenset).
- ✅ **Bug: `acceptInvitation()` ignorerer status-update fejl** (LUKKET v8.88) (b-utils.js:1167). Join-insert ER checket (godt), men `bubble_invitations`-status→'accepted' er ikke → bruger joined men invitation staar pending hvis update fejler. Partial-success. **Fix (LOW risk):** check error, returnér partial. (Mindre alvorlig end ekstern framing — join'et er sikret.)
