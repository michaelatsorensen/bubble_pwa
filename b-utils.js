...
// ── Member row: used in bubble members, event report, invite list ──
// options: { onClick, showLive, showRole, showKick, roleLabel, statusText, liveBadge }
function renderMemberRow(profile, color, options) {
  var opts = options || {};
  var name = escHtml(profile.name || 'Ukendt');
  var title = escHtml(profile.title || '');
  var avatar = renderAvatar(profile.name, color, profile.avatar_url, 36);
  var liveDot = opts.showLive ? '<span class="live-dot"></span>' : '';
  var liveBadge = opts.liveBadge || '';
  var status = opts.statusText ? '<div class="chat-member-status">' + opts.statusText + '</div>' : (title ? '<div class="chat-member-status">' + title + '</div>' : '');
  var rightSide = '';
  if (opts.showRole) rightSide = '<span class="chat-member-role">' + (opts.roleLabel || 'Ejer') + '</span>';
  else if (opts.showKick) rightSide = '<button class="bc-kick-btn" onclick="event.stopPropagation();bcShowKickConfirm(this,\'' + profile.id + '\',\'' + name.replace(/'/g, '') + '\')" title="Fjern fra boble">' + icon('x') + '</button>';

  return '<div class="chat-member-row" data-member-uid="' + profile.id + '"' +
    (opts.onClick ? ' onclick="' + opts.onClick + '"' : '') + '>' +
    '<div class="chat-member-avatar" style="background:' + color + '">' + (profile.avatar_url ? '<img src="' + escHtml(profile.avatar_url) + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%">' : escHtml((profile.name || '?').split(' ').map(function(w){return w[0];}).join('').slice(0,2).toUpperCase())) + liveDot + '</div>' +
    '<div style="flex:1;min-width:0"><div class="chat-member-name">' + name + ' ' + liveBadge + '</div>' + status + '</div>' +
    rightSide +
    '</div>';
}

// ── Bubble card: used in home, discover, mine bobler, events tab ──
// options: { showEdit, showLiveDot, subtitle, badge, action }
function renderBubbleCard(bubble, options) {
  var opts = options || {};
  var action = opts.action || ('data-action="openBubble" data-id="' + bubble.id + '"');
  var emojiHtml = bubble.icon_url
    ? '<img src="' + escHtml(bubble.icon_url) + '" style="width:1.4rem;height:1.4rem;border-radius:4px;object-fit:cover">'
    : (typeof bubbleEmoji === 'function' ? bubbleEmoji(bubble.type) : '🫧');
  var colorBg = typeof bubbleColor === 'function' ? bubbleColor(bubble.type, 0.15) : 'rgba(124,92,252,0.12)';
  var colorFg = typeof bubbleColor === 'function' ? bubbleColor(bubble.type, 0.9) : 'var(--accent)';
  var subtitle = opts.subtitle || ((bubble.type_label || bubble.type) + (bubble.location ? ' · ' + escHtml(bubble.location) : ''));
  var badge = opts.badge || '';
  var right = '';
  if (opts.showEdit) right += '<button class="btn-sm btn-ghost" data-action="openEditBubble" data-id="' + bubble.id + '" onclick="event.stopPropagation()" style="font-size:0.8rem;padding:0.3rem 0.5rem">' + icon('edit') + '</button>';
  if (opts.showLiveDot) right += '<div class="live-dot"></div>';
  var chevron = opts.showChevron ? '<div style="font-size:0.88rem;color:var(--muted)">›</div>' : '';

  return '<div class="card flex-row-center" style="padding:0.85rem 1.1rem;margin-bottom:0.4rem;cursor:pointer" ' + action + '>' +
    '<div class="bubble-icon" style="background:' + colorBg + ';color:' + colorFg + '">' + emojiHtml + '</div>' +
    '<div style="flex:1;min-width:0">' +
    '<div class="fw-600 fs-09">' + escHtml(bubble.name) + ' ' + badge + '</div>' +
    '<div class="fs-075 text-muted">' + subtitle + '</div>' +
    '</div>' +
    (right ? '<div style="display:flex;gap:0.4rem;align-items:center">' + right + '</div>' : '') +
    chevron +
    '</div>';
}
