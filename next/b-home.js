...
async function showDeepLinkModal(type, targetId) {
  try {
    ...
    var iconBg = type === 'contact' ? 'linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210))' : (type === 'event' ? 'rgba(46,207,207,0.15)' : 'rgba(100,180,230,0.15)');
    ...
    ov.innerHTML =
      '<div style="background:rgba(23,15,52,0.94);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-radius:20px;padding:2rem 1.5rem 1.5rem;width:100%;max-width:320px;text-align:center;border:0.5px solid rgba(255,255,255,0.1)">' +
        ...
        (subtitle ? '<div style="font-size:0.8rem;color:rgba(255,255,255,0.4);margin-bottom:1.25rem">' + escHtml(subtitle) + '</div>' : '<div style="margin-bottom:1.25rem"></div>') +
        '<button id="dl-action-btn" style="width:100%;padding:0.8rem;border-radius:12px;border:none;background:rgba(100,180,230,0.18);border:0.5px solid rgba(100,180,230,0.25);color:rgba(255,255,255,0.9);font-size:0.92rem;font-weight:700;font-family:inherit;cursor:pointer;margin-bottom:0.5rem">' + escHtml(actionLabel) + ' →</button>' +
...
async function _showSavedContactModal(contactId) {
  try {
    ...
    ov.innerHTML =
      '<div style="background:rgba(23,15,52,0.94);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-radius:20px;padding:2rem 1.5rem 1.5rem;width:100%;max-width:320px;text-align:center;border:0.5px solid rgba(255,255,255,0.1)">' +
        '<div style="width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210));display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;box-shadow:0 4px 12px rgba(100,180,230,0.25)">' + avHtml + '</div>' +
        '<div style="font-size:1.15rem;font-weight:800;color:var(--text);margin-bottom:0.2rem">' + escHtml(name) + '</div>' +
        (subtitle ? '<div style="font-size:0.8rem;color:rgba(255,255,255,0.4);margin-bottom:0.3rem">' + escHtml(subtitle) + '</div>' : '') +
        '<div style="display:flex;align-items:center;justify-content:center;gap:0.4rem;margin-bottom:1.25rem">' +
        ...
        '</div>' +
        '<button id="sc-view-btn" style="width:100%;padding:0.8rem;border-radius:12px;border:none;background:rgba(100,180,230,0.18);border:0.5px solid rgba(100,180,230,0.25);color:rgba(255,255,255,0.9);font-size:0.92rem;font-weight:700;font-family:inherit;cursor:pointer;margin-bottom:0.5rem">Se profil →</button>' +
...
function showWelcomeCard() {
  ...
  el.innerHTML =
    '<div style="background:rgba(23,15,52,0.82);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:0.5px solid rgba(255,255,255,0.06);border-radius:14px;padding:1rem 1.1rem;position:relative">' +
      '<button onclick="dismissWelcomeCard()" style="position:absolute;top:0.6rem;right:0.7rem;background:none;border:none;cursor:pointer;color:rgba(255,255,255,0.3);font-size:1rem;line-height:1;padding:0.2rem;font-family:inherit" aria-label="Luk">×</button>' +
      '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.5rem">' +
        '<div style="width:28px;height:28px;color:rgba(100,180,230,0.8);flex-shrink:0">' + ico('bubble') + '</div>' +
        '<div style="font-size:0.85rem;font-weight:800;color:rgba(255,255,255,0.95)">Velkommen til Bubble' + (currentProfile.name ? ', ' + currentProfile.name.split(' ')[0] : '') + '!</div>' +
      '</div>' +
      '<div style="font-size:0.75rem;color:rgba(255,255,255,0.45);line-height:1.55;margin-bottom:0.75rem">' +
      ...
      '<div style="display:flex;gap:0.5rem;align-items:center">' +
        '<button onclick="dismissWelcomeCard();goTo(\'screen-profile\');setTimeout(function(){profSwitchTab(\'dashboard\')},200)" style="flex:1;padding:0.55rem;border-radius:10px;font-size:0.78rem;font-weight:700;font-family:inherit;cursor:pointer;background:rgba(100,180,230,0.18);border:0.5px solid rgba(100,180,230,0.25);color:rgba(255,255,255,0.9)">Udfyld profil →</button>' +
        '<button onclick="dismissWelcomeCard()" style="padding:0.55rem 0.8rem;border-radius:10px;font-size:0.78rem;font-weight:600;font-family:inherit;cursor:pointer;background:none;border:0.5px solid rgba(255,255,255,0.12);color:rgba(255,255,255,0.5)">Senere</button>' +
      '</div>' +
    '</div>';
...
function _updateProfileStrengthBar(strength) {
  ...
  if (lbl) {
    ...
    else if (strength >= 50) { lbl.textContent = 'God profil'; lbl.style.color = 'var(--isbla)'; }
...
function openSetupWorkplaceSheet() {
  ...
  sheet.innerHTML = '<div style="padding:1.2rem 1rem">' +
    ...
    '<div style="font-size:0.78rem;color:rgba(255,255,255,0.4);margin-bottom:1rem">Andre kan se din arbejdsplads på din profil</div>' +
...
async function _bbLoadPendingInvites() {
  ...
  try {
    ...
    invites.forEach(function(inv) {
      ...
      html += '<div class="card" style="padding:0.6rem 0.8rem;margin-bottom:0.4rem;border-left:2px solid rgba(100,180,230,0.3)" id="bb-inv-' + inv.id + '">';
      ...
      if (b.name) html += '<div style="font-size:0.62rem;color:rgba(255,255,255,0.25)">' + icon('bubble') + ' ' + escHtml(b.name) + '</div>';
      html += '</div>';
      html += '<div style="display:flex;gap:0.3rem;flex-shrink:0">';
      html += '<button class="btn-sm" style="padding:0.3rem 0.6rem;font-size:0.65rem;font-weight:600;background:rgba(100,180,230,0.18);border:0.5px solid rgba(100,180,230,0.25);color:white;border:none;border-radius:var(--radius-xs);cursor:pointer;font-family:inherit" onclick="event.stopPropagation();bbAcceptInvite(\'' + inv.id + '\',\'' + inv.from_user_id + '\')">Accepter</button>';
  ...
  try {
    ...
    function _goLiveBtn(ev, isMember) {
      ...
      if (win === 'before') {
        ...
        return '<span style="font-size:0.55rem;color:rgba(255,255,255,0.25)">' + t('live_starts') + ' ' + dateStr + '</span>';
      }
      if (win === 'after') return '<span style="font-size:0.55rem;color:rgba(255,255,255,0.25)">' + t('live_ended') + '</span>';
    ...
    parentNets.forEach(function(net) {
      ...
      html += '<div style="font-size:0.62rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;gap:3px;flex-wrap:wrap">' + visIcon(net.visibility) + mc + ' ' + t('bb_members') + (badgeText ? ' \u00B7 ' + badgeText : '') + '</div>';
      html += '</div>';
      ...
      if (totalChildren > 0) {
        ...
        childNets.forEach(function(cn) {
          ...
          html += '<div style="font-size:0.58rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;gap:3px">' + visIcon(cn.visibility) + cnMc + ' ' + t('bb_members_short') + (cnEvents.length > 0 ? ' \u00B7 ' + cnEvents.length + ' ' + t('bb_events_count') : '') + '</div>';
          html += '</div>';
          ...
          if (hasChildren) {
            ...
            cnAdopted.forEach(function(gc) {
              ...
              html += '<div style="font-size:0.55rem;color:rgba(255,255,255,0.25)">' + visIcon(gc.visibility) + gcMc + ' medl.</div></div>';
            ...
            cnEvents.forEach(function(ev) {
              ...
              html += '<div style="font-size:0.55rem;color:rgba(255,255,255,0.25)">' + dateStr + (evMc > 0 ? ' \u00B7 ' + evMc + ' ' + t('bb_attendees') : '') + '</div></div>';
        ...
        childEvents.forEach(function(ev) {
          ...
          html += '<div style="font-size:0.58rem;color:rgba(255,255,255,0.25)">' + visIcon(ev.visibility) + dateStr + (evMc > 0 ? ' \u00B7 ' + evMc + ' ' + t('bb_attendees') : '') + '</div></div>';
          html += _unreadDot(ev.id) + (_goLiveBtn(ev, evIsMember) || '<div class="bb-tree-go">\u203A</div>');
          html += '</div></div>';
        });
    ...
    trueOrphans.forEach(function(net) {
      ...
      if (ghost) {
        ...
        html += '<div style="font-size:0.62rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;gap:3px">' + visIcon(ghost.visibility) + ghostMc + ' ' + t('bb_members') + '</div>';
        html += '</div></div>';
      ...
      html += '<div style="font-size:' + (ghost ? '0.58' : '0.62') + 'rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;gap:3px">' + visIcon(net.visibility) + mc + ' ' + t('bb_members') + '</div>';
      html += '</div>';
      html += _unreadDot(net.id);
...
async function loadMyEvents() {
  try {
    ...
    if (upcoming.length > 0) {
      html += '<div style="font-size:0.68rem;font-weight:700;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:0.4rem">'+t('misc_upcoming')+'</div>';
    ...
    if (past.length > 0) {
      ...
      html += '<div onclick="var c=this.nextElementSibling;var a=c.style.display===\'none\';c.style.display=a?\'block\':\'none\';this.querySelector(\'.bb-acc-chev\').style.transform=a?\'rotate(90deg)\':\'rotate(0)\';_bbAccordionOpen[\'' + accId + '\']=a" style="display:flex;align-items:center;gap:0.5rem;padding:0.6rem 0.2rem;margin-top:0.8rem;cursor:pointer;border-top:1px solid var(--glass-border)">' +
        '<span class="bb-acc-chev" style="transition:transform 0.2s;transform:' + (isOpen ? 'rotate(90deg)' : 'rotate(0)') + ';color:rgba(255,255,255,0.25);font-size:12px"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M9 6l6 6-6 6"/></svg></span>' +
        '<span style="font-size:0.72rem;font-weight:700;color:rgba(255,255,255,0.25);text-transform:uppercase;letter-spacing:0.04em">' + t('misc_past') + ' (' + past.length + ')</span>' +
      '</div>';
...
function _bbEventCard(e, parentMap, gpMap, isPast) {
  ...
  return '<div class="bb-tree-evt" data-action="openBubble" data-id="' + e.id + '" style="margin-bottom:0.45rem;' + (isPast ? 'opacity:0.5;' : '') + '">' +
    ...
    '<div style="font-size:0.62rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;flex-wrap:wrap;gap:2px;margin-top:2px">' + visIcon(e.visibility) + dateStr + '</div>' +
    breadcrumb +
...
async function loadTopMatches() {
  try {
    ...
    // Render
    var avColors = ['linear-gradient(135deg,#2ECFCF,#22B8CF)','linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210))','linear-gradient(135deg,#E879A8,#EC4899)','linear-gradient(135deg,#F59E0B,#EAB308)','linear-gradient(135deg,#1A9E8E,#10B981)'];

    list.innerHTML = scored.map(function(s, i) {
      var p = s.profile;
      var ini = (p.name||'?').split(' ').map(function(w){return w[0];}).join('').slice(0,2).toUpperCase();
      var avHtml = p.avatar_url ?
        '<div style="width:40px;height:40px;border-radius:50%;overflow:hidden;flex-shrink:0;border:2px solid rgba(255,255,255,0.15)"><img src="' + escHtml(p.avatar_url) + '" style="width:100%;height:100%;object-fit:cover"></div>' :
      ...
      var sharedTags = s.shared.slice(0, 3).map(function(t) {
        return '<span style="font-size:0.58rem;padding:0.1rem 0.4rem;background:rgba(255,255,255,0.06);color:var(--isbla);border-radius:99px;font-weight:600">' + escHtml(t) + '</span>';
      }).join('');
      ...
      return '<div style="background:var(--glass-bg-strong);border:1px solid var(--glass-border-subtle);border-radius:12px;padding:0.6rem 0.75rem;display:flex;align-items:center;gap:0.65rem;cursor:pointer" onclick="openPerson(\'' + p.id + '\',\'screen-home\')">' +
        ...
        '<div style="flex:1;min-width:0">' +
          ...
          '</div>' +
          '<div style="font-size:0.68rem;color:rgba(255,255,255,0.4);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + escHtml(p.workplace || '') + (p.title && p.workplace ? ' \u00b7 ' + escHtml(p.title) : escHtml(p.title || '')) + '</div>' +
...
function bubbleCard(b, joined) {
  ...
  var upLabel = ups > 0 ? `<div class="fs-065" style="color:var(--isbla);display:flex;align-items:center;gap:0.15rem">${icon('rocket')}<span style="font-weight:700">${ups}</span></div>` : '';

  // Contact avatars (from Discover)
  var contactHtml = '';
  var contacts = b._contacts || [];
  if (contacts.length > 0) {
    var avColors = ['linear-gradient(135deg,#2ECFCF,#22B8CF)','linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210))','linear-gradient(135deg,#E879A8,#EC4899)','linear-gradient(135deg,#F59E0B,#EAB308)','linear-gradient(135deg,#1A9E8E,#10B981)','linear-gradient(135deg,#8B5CF6,#A855F7)','linear-gradient(135deg,#3B82F6,#6366F1)','linear-gradient(135deg,#EF4444,#F97316)','linear-gradient(135deg,#06B6D4,#0EA5E9)','linear-gradient(135deg,#D946EF,#C026D3)'];
  ...
  return `<div class="card flex-row-center" data-action="openBubble" data-id="${b.id}">
    ...
    <div style="flex:1;min-width:0">
      ...
      <div style="font-size:0.68rem;color:rgba(255,255,255,0.25);display:flex;align-items:center;gap:0.25rem;flex-wrap:wrap">${visIcon(b.visibility)} ${escHtml(b.type_label || b.type)} ${b.location ? '<span>·</span> <span>' + escHtml(b.location) + '</span>' : ''}</div>
      ${eventDateHtml}
      ${parentRef}
      ${childPills}
      ${contactHtml}
    ...
    <div class="flex-col-end" style="align-items:flex-end;gap:0.15rem">
      ${memberLabel}
      ${upLabel}
      ${joined ? '<div class="live-dot"></div>' : '<div class="fs-09" style="color:var(--isbla)">+</div>'}
    </div>
  </div>`;
}
...
function updateAnonToggle() {
  ...
  toggle.style.background = isAnon ? 'var(--isbla)' : 'var(--border-1)';
...
function toggleRadarFilters() {
  ...
  if (icon) {
    if (_radarFiltersOpen) {
      icon.innerHTML = '<line x1="6" y1="6" x2="18" y2="18"/><line x1="18" y1="6" x2="6" y2="18"/>';
    } else {
...
function _homeDrawProxRings(canvas) {
  ...
  var zones = [
    {r:0.10,fill:'rgba(23,15,52,0.06)'},{r:0.26,fill:'rgba(23,15,52,0.04)'},
    {r:0.42,fill:'rgba(23,15,52,0.025)'},{r:0.58,fill:'rgba(23,15,52,0.018)'},
    {r:0.74,fill:'rgba(23,15,52,0.012)'},{r:0.90,fill:'rgba(23,15,52,0.006)'}
  ];
  ...
  for (var i = 0; i < zones.length; i++) {
    ctx.beginPath(); ctx.arc(cx,cy,zones[i].r*maxR,0,Math.PI*2);
    ctx.strokeStyle = 'rgba(23,15,52,0.06)'; ctx.lineWidth = 1; ctx.stroke();
  }
  var g = ctx.createRadialGradient(cx,cy,0,cx,cy,zones[0].r*maxR);
  g.addColorStop(0,'rgba(23,15,52,0.08)'); g.addColorStop(1,'rgba(23,15,52,0)');
...
function renderHomeDartboard() {
  ...
  if (profiles.length === 0) {
    ...
    if (_homeViewMode === 'live') {
      av.innerHTML = '<div class="dartboard-empty" style="position:absolute;top:50%;left:50%;transform:translate(-50%, calc(-100% - 40px));text-align:center;padding:0.6rem 1.2rem;background:rgba(255,255,255,0.85);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border-radius:12px;border:1px solid var(--glass-border-subtle);box-shadow:0 2px 8px rgba(30,27,46,0.06);white-space:nowrap">' +
        ...
        '<div style="font-size:0.68rem;color:rgba(255,255,255,0.25);margin-top:0.15rem">' + t('home_waiting') + '</div>' +
        '</div>';
      ...
      av.innerHTML = '<div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;padding:0 2rem">' +
        ...
        '<div style="font-size:0.72rem;color:rgba(255,255,255,0.25);text-align:center;line-height:1.4">' + t('home_radar_empty_desc') + '</div>' +
  ...
  var colors = (typeof proxColors !== 'undefined' && proxColors) ? proxColors : [
    'linear-gradient(135deg,#2ECFCF,#22B8CF)','linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210))',
...
function renderHomeTrayList() {
  ...
  var colors = proxColors || ['linear-gradient(135deg,rgb(100,180,230),rgb(70,150,210))'];
  list.innerHTML = sorted.map(function(p, i) {
    ...
    return '<div onclick="openRadarPerson(\'' + p.id + '\')" style="display:flex;align-items:center;gap:0.75rem;padding:0.7rem 0.85rem;margin-bottom:0.4rem;border-radius:14px;background:rgba(255,255,255,0.06);border:0.5px solid rgba(255,255,255,0.06);cursor:pointer;transition:transform 0.3s cubic-bezier(0.34,1.56,0.64,1);-webkit-tap-highlight-color:transparent" ontouchstart="this.style.transform=\'scale(0.97)\'" ontouchend="this.style.transform=\'scale(1)\'">' +
      avHtml +
      '<div style="flex:1;min-width:0">' +
        '<div style="display:flex;align-items:center;gap:0.4rem">' +
